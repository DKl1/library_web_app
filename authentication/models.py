...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    password = models.CharField(default=None, max_length=255, blank=True)
    ...
    @staticmethod
    def create(email, password, first_name=None, middle_name=None, last_name=None, role=0, is_staff=False):
        """
        ...
        if len(first_name) <= 20 and len(middle_name) <= 20 and len(last_name) <= 20 and len(email) <= 100 and len(
            ...
            custom_user = CustomUser(email=email, password=password, first_name=first_name, middle_name=middle_name,
                                     last_name=last_name, role=role, is_staff=is_staff)
            custom_user.save()
            return custom_user
        return None
